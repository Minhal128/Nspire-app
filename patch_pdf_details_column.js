...
d = d.replace(
  ...
  "    <td class=\"la\">${isGC ? '-' : (def.deficiencyName && def.deficiencyName !== 'Deficiency' && def.deficiencyName !== 'General Comment' ? `<b>${esc(def.deficiencyName)}</b><br/>` : '') + esc(def.deficiencyDetails || 'No details available')}</td>"
);
