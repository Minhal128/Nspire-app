...
import {
  getEnhancedReportHTML as getWebReportHTML,
  setReportLogo,
...
function generateEnhancedNSPIREReportHTML(
  ...
  // The finished report is the web build's, generated by its own module so the
  // two products produce the same document. Only two things are adapted for the
  // device: the logo is embedded rather than fetched from /logo.png, and each
  // deficiency's imageUri is swapped for the base64 the app preloaded, because a
  // WebView cannot reach a remote URL while the inspector is offline.
  setReportLogo(logoBase64);

  const reportForWeb = imageMap.size === 0 ? report : {
    ...report,
    deficiencies: (report.deficiencies || []).map((def: any) => {
      const raw = def?.imageUri || def?.imageUrl;
      const embedded = raw ? imageMap.get(raw) : undefined;
      return embedded ? { ...def, imageUri: embedded } : def;
    }),
  };

  return getWebReportHTML(reportForWeb as any, options as any);
}
